...
namespace LiveSplit.View
{
    public partial class TimerForm : Form
    {
        ...
        {
            foreach (var item in comparisonMenuItem.DropDownItems)
            {
        ...
        void Race_Click(object sender, EventArgs e)
        {
            if (ShowSRLRules())
            {
                ...
                var form = new SpeedRunsLiveForm(Model, raceId);
                this.TopMost = false;
                form.Show(this);
        ...
        void NewRace_Click(object sender, EventArgs e)
        {
            if (ShowSRLRules())
            {
                ...
                if (result == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    var form = new SpeedRunsLiveForm(Model, gameName, id, gameCategory);
                    form.Show(this);
                }
                this.TopMost = CurrentState.LayoutSettings.AlwaysOnTop;
            }
        ...
        void CurrentState_OnReset(object sender, EventArgs e)
        {
            RegenerateComparisons();
            Action x = () =>
            {
        ...
        private void UpdateRecentSplits()
        {
            ...
            foreach (var item in Settings.RecentSplits.Reverse())
            {
        ...
        void hook_KeyOrButtonPressed(object sender, KeyOrButton e)
        {
            Action action = () =>
            {
                if ((this.Focused || Settings.GlobalHotkeysEnabled) && !ResetMessageShown && !IsInDialogMode)
                {
                ...
                if (this.Focused && !ResetMessageShown && !IsInDialogMode)
                {
        ...
        {
            var warnUser = false;
            TimeSpan? currentSegment = TimeSpan.Zero;
            TimeSpan? previousSplitTime = TimeSpan.Zero;
            foreach (var split in CurrentState.Run)
            {
                if (split.RTASplitTime != null)
                {
                    currentSegment = split.RTASplitTime - previousSplitTime;
                    previousSplitTime = split.RTASplitTime;
                    if (split.BestSegmentTime == null || currentSegment < split.BestSegmentTime)
                    {
                        warnUser = true;
                        break;
                    }
            ...
            if (!warnUser && (CurrentState.Run.Last().RTASplitTime != null && CurrentState.Run.Last().PersonalBestSplitTime == null) || CurrentState.Run.Last().RTASplitTime < CurrentState.Run.Last().PersonalBestSplitTime)
                warnUser = true;
            if (warnUser)
            {
                DontRedraw = true;
        ...
        {
            var menuItem = new ToolStripMenuItem(name) { Checked = name == CurrentState.CurrentComparison };
